...
# client.follow("gilet_jauni") # Follow ce que tu veux

=begin # Like twit
    tweets = client.search("#BonjourTwitter").take(5)
    tweets.collect { |twit| client.favorite(twit.id)}

=end

=begin # Like User ID
    tweets = client.search("#BonjourTwitter").take(5)
    tweets.collect { |twit| client.follow(twit.user.id)}

=end

def stream
=begin # Stream Coffee or Tea tweet
	topics = ["#BonjourTwitter"]
	client_stream.filter(track: topics.join(",")) do |object|
	  puts object.text if object.is_a?(Twitter::Tweet)
	end
=end

	client_stream.sample do |object|
	  puts object.text if object.is_a?(Twitter::Tweet)
	end
